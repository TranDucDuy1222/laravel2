    ...
    <div class="form-search">
        <form @submit.prevent="handleSearch">
            <div class="d-flex align-items-center ms-2">
                ...
                <input v-model="searchQuery" class="custom-input border-0 ms-2" type="search" placeholder="Nhập từ khoá..." @input="debouncedSearch">
            </div>
        </form>
...
export default {
    ...
    methods: {
        handleSearch() {
            window.location.href = `/ket-qua?tim-kiem=${encodeURIComponent(this.searchQuery)}`;
        },
        search() {
            if (this.searchQuery) {
        ...
        debouncedSearch: debounce(function (event) {
            ...
            this.search();
        }, 800) // Thời gian debounce là 800ms (0.8 giây)
    }
};
</script>
